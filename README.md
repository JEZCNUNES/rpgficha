...
const storageSave = async (chars) => {
  try {
    await window.storage.set(STORAGE_KEY, JSON.stringify(chars), true);
  } catch (e) { console.error('save fail', e); }
};

const rollsLoad = async () => {
  try {
    const r = await window.storage.get(ROLLS_KEY, true);
    if (r && r.value) return JSON.parse(r.value);
  } catch (e) {}
  return [];
};
const rollsSave = async (rolls) => {
  try { await window.storage.set(ROLLS_KEY, JSON.stringify(rolls.slice(0, 30)), true); }
  catch (e) {}
};
...
export default function App() {
  ...
  useEffect(() => {
    storageLoad().then(chars => {
      setCharacters(chars);
      setLoading(false);
    });
    // Auto-refresh a cada 15s para detectar mudanças de outros jogadores
    const interval = setInterval(() => {
      storageLoad().then(chars => setCharacters(prev => {
        // só atualiza se realmente mudou
        if (JSON.stringify(prev) !== JSON.stringify(chars)) return chars;
        return prev;
      }));
    }, 15000);
